...
export default function ERCSPage() {
  ...
  const handleScenarioChange = (s: "stable" | "warning" | "critical") => {
    setScenario(s)
    // Auto-navigate to optimise tab when entering warning/critical
    if (s === "warning" || s === "critical") {
      setActiveTab("optimise")
    }
  }
  ...
  const renderTab = () => {
    switch (activeTab) {
      case "dashboard":
        return <DashboardTab scenario={scenario} onScenarioChange={handleScenarioChange} />
      case "simulator":
