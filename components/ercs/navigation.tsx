...
export function Navigation({ activeTab, onTabChange, alertCount = 0, scenario = "stable" }: NavigationProps) {
  const optimiseActive = scenario === "warning" || scenario === "critical"
          ...
          {tabs.map((tab) => {
            ...
            return (
              ...
              >
                ...
                {/* Optimise pulse dot when system is not stable */}
                {isOptimise && optimiseActive && !isActive && (
                  <span className={`relative z-10 w-2 h-2 rounded-full animate-pulse ${
